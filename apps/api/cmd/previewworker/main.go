...
func main() {
	telemetry, err := observability.Setup(context.Background(), observability.PreviewConfigFromEnv())
	if err != nil {
		log.Fatal(err)
	}
	...
	if bucket == "" {
		fatal(logger, errors.New("PRESENTATION_GCS_BUCKET is required"))
	}
	...
	ready.Store(true)
	slog.Info("preview worker started", slog.Int("concurrency", maxWorkers))
	select {
