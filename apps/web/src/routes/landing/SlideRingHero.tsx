...
import { type LandingPlate, randomLandingPlates } from "./landing-plates";
...
export function SlideRingHero() {
	...
	/* sampled once per visit, so the ring is never the same two loads running */
	const [plates] = useState<LandingPlate[]>(() => randomLandingPlates());
	...
	const preview = previewIndex === null ? null : plates[previewIndex];
					...
					>
						<div className="h-full w-full overflow-hidden rounded-lg shadow-2xl ring-1 ring-white/15">
							<img
								...
								alt={`${preview.name} template cover`}
								className="h-full w-full object-cover"
							/>
						</div>
						<p className="mt-4 text-center text-xs tracking-wide text-white/50">{preview.name}</p>
					</div>
				</div>
			)}
		</div>
